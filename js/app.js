...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function(event, ui) {
            ...
            if (!itemData.useable) {
                $("#item-use").css("background", "rgba(182, 182, 182, 0.3");
            }
                ...
                if (itemData.price != null) {
                    ...
                    $(".ui-draggable-dragging")
                        ...
                        .html(
                            itemData.amount +
                            " (" +
                            ((itemData.weight * itemData.amount)).toFixed(1) +
                            ")"
                        );
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                    if ($(this).parent().attr("data-inventory") == "hotbar") {
                ...
                if (itemData.price != null) {
                    ...
                    $(".ui-draggable-dragging")
                        ...
                        .html(
                            dragAmount +
                            " (" +
                            ((itemData.weight * dragAmount)).toFixed(1) +
                            ")"
                        );
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                    if ($(this).parent().attr("data-inventory") == "hotbar") {
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    $("#player-inv-weight").html(
        "⚖️: " +
        (parseInt(totalWeight)).toFixed(2) +
        " / " +
        (playerMaxWeight).toFixed(2)
    );
    if (
        $fromInv.attr("data-inventory").split("-")[0] != "itemshop" &&
        $toInv.attr("data-inventory").split("-")[0] != "itemshop" &&
        ...
        $("#other-inv-weight").html(
            "⚖️: " +
            (parseInt(totalWeightOther)).toFixed(2) +
            " / " +
            (otherMaxWeight).toFixed(2)
        );
    }
    ...
    if ($toSlot < 6) {
        $toInv
            ...
            .html(
                ...
                fromData.amount +
                " (" +
                ((fromData.weight * fromData.amount)).toFixed(1) +
                ')</p></div><div class="item-slot-label"><p>' +
                fromData.label +
                "</p></div>"
            );
    ...
    if ($fromSlot < 6) {
        $fromInv
            ...
            .html(
                ...
                toData.amount +
                " (" +
                ((toData.weight * toData.amount)).toFixed(1) +
                ')</p></div><div class="item-slot-label"><p>' +
                toData.label +
                "</p></div>"
            );
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        ...
                        .html(
                            ...
                            newData.amount +
                            " (" +
                            ((newData.weight * newData.amount)).toFixed(1) +
                            ")</p></div>" +
                            ItemLabel
                        );
                } 
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv
                            ...
                            .html(
                                ...
                                newDataFrom.amount +
                                " (" +
                                ((newDataFrom.weight * newDataFrom.amount)).toFixed(
                                    1
                                ) +
                                ")</p></div>" +
                                ItemLabel
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        ...
                        .html(
                            ...
                            newDataTo.amount +
                            " (" +
                            ((newDataTo.weight * newDataTo.amount)).toFixed(1) +
                            ")</p></div>" +
                            ItemLabel
                        );
                // } else if ($toSlot == 41 && $toInv.attr("data-inventory") == "player") {
    ...
    Inventory.Open = function(data) {
        ...
        if (data.other != null) {
            ...
            if (
                ...
                $("#other-inv-weight").html(
                    "⚖️: " +
                    (totalWeightOther).toFixed(2) +
                    " / " +
                    (data.other.maxweight).toFixed(2)
                );
            }
            ...
            $("#other-inv-weight").html(
                ...
                (totalWeightOther).toFixed(2) +
                " / " +
                (Inventory.dropmaxweight).toFixed(2)
            );
